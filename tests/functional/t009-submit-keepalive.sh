#!/usr/bin/env bash
#
# t009-submit-keepalive.sh - Bootstrap a KeepAlive job and oracle LaunchAgent.
#
# Submits helloworld under submit (KeepAlive), asserts launchctl print
# shows type = LaunchAgent, then boots the job out via launchctl.
#
set -euo pipefail
IFS=$'\n\t'
...
main() {
	...
	local -r label="com.xspawn.test.submit.$$"
