#!/usr/bin/env bash
#
# t003-oneshot-missing-label.sh - oneshot without -l must print usage.
#
# Invokes oneshot with a programme but no label. Expects non-zero exit
...
main() {
	...
	err="$("${SPAWN_BIN}" oneshot -- "${HELLOWORLD_BIN}" 0 2>&1)" || \
		rc=$?
