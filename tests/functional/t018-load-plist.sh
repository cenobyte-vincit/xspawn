#!/usr/bin/env bash
#
# t018-load-plist.sh - load -p must bootstrap an existing plist.
#
# Writes a caller-owned job.plist, loads it with -p, asserts
# type = LaunchAgent, and asserts the plist is still on disk.
#
set -euo pipefail
IFS=$'\n\t'
...
main() {
	...
	local -r label="com.xspawn.test.load.$$"
...
EOF
	...
	"${SPAWN_BIN}" load -p "${plist}"

	[ ! -f "${plist}" ] && \
		errx "load deleted the caller plist: ${plist}"

	rc=0
	print_out="$(launchctl print "${domain}" 2>&1)" || rc=$?
	[[ "${rc}" -ne 0 ]] && \
		errx "launchctl print failed after load for ${label}"
